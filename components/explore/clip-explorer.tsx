...
export function ClipExplorer({
  ...
  const particleClips =
    particle === "" ? clips : clips.filter((clip) => clip.particle === particle);
  ...
  const visibleClips =
    meaning === ""
      ...
      : particleClips.filter(
          (clip) => clip.communicative_function === meaning,
        );
  ...
  return (
    <main className="explorer-page">
      ...
      <section className="particle-rail" aria-label="Filter by final particle">
        <button
          type="button"
        ...
        {TARGET_PARTICLES.map(({ token, pinyin }) => {
          const count = clips.filter((clip) => clip.particle === token).length;
          return (
            <button
              type="button"
